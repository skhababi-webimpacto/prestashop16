...
{if !isset($request)}
	...
	{/if}
{/if}
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm form-horizontal pull-right hidden-xs ">
	<div class="select">
		...
		<select class="form-control selectProductSort" id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}">
			...
			<!-- Added by Said -->
			<option value="mostsold:desc" {if $orderby eq 'mostsold' AND $orderway eq 'desc'}selected="selected"{/if}>{l s='Most sold: Highest first'}</option>
			<option value="mostsold:asc" {if $orderby eq 'mostsold' AND $orderway eq 'asc'}selected="selected"{/if}>{l s='Most sold: Lowest first'}</option>
		</select>
