	...
	if (view == 'list')
	{
		...
		$('.product_list > div').each(function(index, element) {
			...
			html = '<div class="product-container product-block text-center"><div class="row">';
				html += '<div class="left-block col-xs-5 col-sp-12 col-md-4">' + $(element).find('.left-block').html() + '</div>';
				html += '<div class="right-block col-xs-7 col-sp-12 col-md-8">' + $(element).find('.right-block').html() + '</div>';
			html += '</div></div>';
		$(element).html(html);
		});		
