...
class Datasample {
    ...
    public function processImport($moduleName = ''){
        ...
        if( !$data || !isset($data->modules)) return false;
        if(!$moduleName)
        ...
        foreach ($moduleList as $module) {
            if(file_exists(_PS_ALL_THEMES_DIR_.$this->_theme_dir.'samples/'.$module.'.xml')){
                ...
                if(isset($content->sql) && $content->sql){
                    ...
                    {
                        $query = trim($query);if (!$query) continue;
                ...
                if(isset($content->hooks) && $content->hooks){
                    ...
                    if ($moduleObj && $moduleObj->id){
                            ...
                            {   
                                ...
                                $exceptions = (isset($row->attributes()->exceptions) ? explode(',', strval($row->attributes()->exceptions)) : array());
                ...
                foreach($content->fields as $fields){
                    ...
                    if(isset($fields->attributes()->objectName) && $fields->attributes()->objectName){
                        if(!$this->includeObjModel($module, (string)$fields->attributes()->objectFile)) continue;
    ...
    public function importData($content, $objectMName, $searchField, $searchValue, $moduleName, $parentID = 0) {
        ...
        else $realContent = $content->children();
        ...
        foreach ($realContent as $parrentField) {
            ...
            if (isset($parrentField->child_field)) {
                if(isset($parrentField->child_field->attributes()->field)) $this->_parentField = (string)$parrentField->child_field->attributes()->field;
    ...
    public function deleteDataOfModByShop($objectMName, $searchField, $searchValue, $moduleName){
        ...
        if (!count(Db::getInstance()->executeS('SHOW TABLES LIKE \''.$tableName.'\''))) return false;
...
<dataSample>';
                    ...
                    if($moduleXml->config_prefix){
                        ...
                        }else $configs = $this->getConfigByName((string)$moduleXml->config_prefix);
...
$this->_content .= '                                                   
        <config>';
$this->_content .= '                                                   
            <name><![CDATA['.$config["name"].']]></name>';
$this->_content .= '                                                   
            <value><![CDATA['.$config["value"].']]></value>';
$this->_content .= '                                                   
        </config>';
                            }
                        }
$this->_content .= '                                                   
                    ...
                    foreach ($moduleXml->fields->children() as $field){
                            if(!isset($field->table_name)) 
                                if(!$this->includeObjModel($moduleXml->name, $field->objectMFile)) continue;
                                

if(isset($field->table_name))                         
$this->_content .= '                                                   
    <fields table_name="'.$field->table_name.'" searchField="'.$field->searchField.'">';
else
$this->_content .= '                                                   
    <fields objectName="'.$field->objectMName.'" objectFile="'.$field->objectMFile.'" searchField="'.$field->searchField.'">';
                            if(isset($field->table_name))
...
$this->_content .= '
    <sql><![CDATA['.$this->_createTable.']]></sql>';
    //export hook
$this->_content .= '
    <hooks>';
    ...
    foreach ($hook_list as $hook){
        $this->_content .= '
        <hook module="'.(string)$moduleXml->name.'" hook="'.$hook["name_hook"].'" position="'.$hook["position"].'" exceptions="'.$hook["exceptions"].'"/>';
    }
$this->_content .= '
    </hooks>';
$this->_content .= '
        ...
        <field>';
            ...
            <'.$key.' type="string"><![CDATA['.$value.']]></'.$key.'>';
                    ...
                    if($field->table_name=="leohook" && $key=="id_module"){
                        ...
                        $this->_content .= '
            <'.$key.' type="module_name">'.$resultModule.'</'.$key.'>';
                        ...
                        $this->_content .= '
            <'.$key.' type="hook_name">'.$resultHook.'</'.$key.'>';          
                    }
        ...
        <field>';
            ...
            <'.$ke.'>';
                    foreach($this->_languages as $langISO => $langID){
                        $this->_content .= '
            <'.$langISO.'><![CDATA['.$obj->{$ke}[$langID].']]></'.$langISO.'>';
                    }
                    $this->_content .= '
            </'.$ke.'>';
                }
            ...
            if($moduleName == 'leobootstrapmenu' && $className == 'Btmegamenu'){
                 ...
                 if(!$this->_paramsMenu){
                     ...
                     foreach( $configMenu as $param )
                        if($param) $this->_paramsMenu[$param->id] = $param;
                 }
                 $this->_content .= '
            <params><![CDATA['.  Tools::jsonEncode($this->_paramsMenu[$obj->id]) . ']]></params>';
            }
            ...
            if(isset($value['child']) && $value['child']){
                $this->_content .= '
            <child_field field="'.(string)$field->treeField.'">';
            ...
            if(isset($field->field) && $field->field && $obj->id){
                foreach ($field->field as $f){
                    if(!$this->includeObjModel($moduleName, $f->objectMFile)) return;
                    //build struct of sub table
                    $this->_content .= '
            <fields objectName="'.$f->objectMName.'" objectFile="'.$f->objectMFile.'" searchField="'.$f->searchField.'">';
    ...
    public function changeLangIsoID($textKey, $type=0){
        ...
        if($type==1){
            if(isset($tmp[2])) $tmp[2] = $this->_languages[$tmp[2]];
            else $tmp[1] = $this->_languages[$tmp[1]];
        }else{
            foreach ($this->_languages as $key => $value) {
                if((!isset($tmp[2]) && $value == $tmp[1]) || (isset($tmp[2]) && $value == $tmp[2])){
                    if(isset($tmp[2])) $tmp[2] = $key;
                    else $tmp[1] = $key;
                    break;
                }
            }
    ...
    public function getModuleList($path = '', $onlyModName = 0){
        if( !file_exists( _PS_ALL_THEMES_DIR_.$this->_theme_dir.'config.xml') )return ;

        $data = simplexml_load_file( _PS_ALL_THEMES_DIR_.$this->_theme_dir.'config.xml' );

        if( !$data || !isset($data->modules))return null;
        $moduleList = array();
    ...
    public function getConfigByName($name){
        ...
        if (Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE'))
            $where .= ' AND `id_shop`='.$this->_id_shop;
        else
            $where .= ' AND `id_shop` IS NULL';
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT name,value FROM `'. _DB_PREFIX_ . 'configuration`' . $where);
    }
    ...
    public function createConfigSample($module, $configPrefix) {
        ...
        }else $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'configuration WHERE name LIKE (\'%' . pSQL($configPrefix) . '%\')';
